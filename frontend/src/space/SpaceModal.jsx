  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose}>
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`${property.name} 공간 보기`}
        onMouseDown={(event) => event.stopPropagation()}
      >
        <button className="modal-close" onClick={onClose} aria-label="공간 보기 닫기">×</button>
        <span className="preset-badge">미리 생성된 모델</span>
        <h2>{property.name} · 공간 보기</h2>
        ...
        <div className="space-tabs">
          <button className={tab === 'plan' ? 'active' : ''} onClick={() => setTab('plan')}>2D 평면도</button>
          <button className={tab === '3d' ? 'active' : ''} onClick={() => setTab('3d')}>3D 공간 보기</button>
        </div>
        {tab === 'plan' ? (
        ...
        <p className="space-warning preset-note">데모용 추정 모델이며 실제 구조와 치수는 다를 수 있습니다.</p>
      </section>
    </div>
  )
}
