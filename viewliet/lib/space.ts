...
export function normalize(
  ...
  const hasLoft =
    ...
    (options.illustrativeDemo ||
      (r.loft.evidence.source === "observed" &&
        r.loft.evidence.photos.some((n) => n >= 1 && n <= photoCount)));
  r.shell = {
    ...r.shell,
    width: w,
    depth: d,
    height: hasLoft
      ? clamp(positive(r.shell.height, 4.2), 3.4, 6.5)
      : clamp(positive(r.shell.height, 2.4), 2, 3.5),
  };
  notes.push(
  ...
  const upperIds = new Set(r.loft?.furnitureIds || []);
  const upperCanonical = new Set<string>();
  r.furniture = r.furniture.flatMap((f, i) => {
    ...
    if (upperIds.has(f.id)) upperCanonical.add(`furniture-${i}`);
    return [
      {
        ...f,
        id: `furniture-${i}`,
    ...
    (f, i, a) =>
      !a
        ...
        .some(
          (g) =>
            g.kind === f.kind &&
            upperCanonical.has(g.id) === upperCanonical.has(f.id) &&
            Math.hypot(g.x - f.x, g.z - f.z) < 0.25,
        ),
  );
  if (!hasLoft) {
    r.loft = null;
    r.furniture = r.furniture.filter((f) => !upperCanonical.has(f.id));
  }
  if (r.loft) {
    const l = r.loft;
    l.evidence = fixEvidence(l.evidence);
    l.x = clamp(l.x * sx, 0, w - 0.8);
    l.z = clamp(l.z * sz, 0, d - 0.8);
    l.width = clamp(positive(l.width * sx, 1.5), 0.8, w - l.x);
    l.depth = clamp(positive(l.depth * sz, 1.5), 0.8, d - l.z);
    l.elevation = clamp(positive(l.elevation, 2.1), 1.9, r.shell.height - 1.2);
    const free = {
      north: l.z,
      south: d - l.z - l.depth,
      west: l.x,
    ...
    const a = l.stairs;
    if (free[a.side] < 0.8) {
      a.side = (Object.keys(free) as (keyof typeof free)[]).sort(
        (a, b) => free[b] - free[a],
      )[0];
      a.evidence = inferred("계단 여유 공간에 맞춰 접근 방향을 추정했습니다.");
    }
    if (free[a.side] < 0.8) {
      r.loft = null;
      r.furniture = r.furniture.filter((f) => !upperCanonical.has(f.id));
      notes.push(
        "직선 계단을 놓을 여유가 없어 복층과 복층 가구는 표시하지 않았습니다.",
      );
    } else {
      const horizontal = a.side === "north" || a.side === "south",
        span = horizontal ? l.width : l.depth;
      a.width = clamp(positive(a.width, 0.7), 0.5, Math.min(1.2, span));
      a.offset = clamp(a.offset * (horizontal ? sx : sz), 0, span - a.width);
      a.run = clamp(positive(a.run, 2), 0.8, free[a.side]);
      a.evidence = fixEvidence(a.evidence);
      l.furnitureIds = r.furniture
        .filter((f) => upperCanonical.has(f.id))
        .map((f) => f.id);
      for (const f of r.furniture)
        if (upperCanonical.has(f.id)) {
          const swap = f.rotation % 180 !== 0;
          const scale = Math.min(
            1,
            (l.width - 0.08) / (swap ? f.depth : f.width),
            (l.depth - 0.08) / (swap ? f.width : f.depth),
          );
          f.width *= scale;
          f.depth *= scale;
          const hx = (swap ? f.depth : f.width) / 2,
            hz = (swap ? f.width : f.depth) / 2;
          f.x = clamp(f.x, l.x + hx + 0.02, l.x + l.width - hx - 0.02);
          f.z = clamp(f.z, l.z + hz + 0.02, l.z + l.depth - hz - 0.02);
          f.height = Math.min(f.height, r.shell.height - l.elevation - 0.1);
        }
      r.assumptions.push(
        "입력 전용면적은 아래층 외곽의 기준으로 사용하고 복층 바닥 면적은 별도로 표시합니다. 매물 면적에 복층이 포함된 경우 차이가 생길 수 있습니다.",
        "계단과 난간은 시각화용 추정입니다. 실제 시공 규격·안전 기준을 나타내지 않습니다.",
      );
    }
  }
...
export function loftStairs(s: Space) {
  const l = s.loft;
  if (!l) return null;
  const a = l.stairs,
    horizontal = a.side === "north" || a.side === "south";
  return {
    x: horizontal
      ? l.x + a.offset
      : a.side === "west"
    ...
    z: horizontal
      ? a.side === "north"
        ...
        : l.z + l.depth
      : l.z + a.offset,
    width: horizontal ? a.width : a.run,
    depth: horizontal ? a.run : a.width,
  };
}
export function levelSpace(s: Space, upper = false): Space {
  const l = s.loft;
  if (!l) return s;
  return {
    ...s,
    furniture: s.furniture.filter(
      (f) => l.furnitureIds.includes(f.id) === upper,
    ),
    partitions: upper
      ? s.partitions.filter((p) => p.height > l.elevation + 0.15)
      : s.partitions,
    openings: upper
      ? s.openings.filter(
          (o) =>
            o.sill + o.height > l.elevation + 0.15 &&
            (["north", "south", "east", "west"].includes(o.wallId) ||
              s.partitions.some(
                (p) => p.id === o.wallId && p.height > l.elevation + 0.15,
              )),
        )
      : s.openings,
    zones: upper
