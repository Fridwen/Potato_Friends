...
export default function RoomView({
  ...
  useEffect(() => {
    ...
    const loft = space.loft;
    if (loft) {
      box(
        loft.x + loft.width / 2,
        loft.elevation - 0.075,
        loft.z + loft.depth / 2,
        loft.width,
        0.15,
        loft.depth,
      ...
      const stair = loftStairs(space)!;
      const a = loft.stairs,
        alongZ = a.side === "north" || a.side === "south",
        count = Math.ceil(loft.elevation / 0.2),
        tread = a.run / count;
      for (let i = 0; i < count; i++) {
        const height = (loft.elevation * (i + 1)) / count;
        const offset =
          a.side === "south" || a.side === "east"
            ? a.run - (i + 0.5) * tread
            : (i + 0.5) * tread;
        box(
          stair.x + (alongZ ? a.width / 2 : offset),
          height / 2,
          stair.z + (alongZ ? offset : a.width / 2),
          alongZ ? a.width : tread,
          height,
          alongZ ? tread : a.width,
          "#bfa37c",
        );
      }
      for (const side of ["north", "south", "west", "east"] as const) {
        const horizontal = side === "north" || side === "south",
          length = horizontal ? loft.width : loft.depth;
        const spans =
          side === a.side
            ? [
                [0, a.offset],
                [a.offset + a.width, length],
              ]
            : [[0, length]];
          ...
          if (end - start > 0.04) {
            ...
            const z = horizontal
              ? side === "north"
                ...
                : loft.z + loft.depth
              : loft.z + (start + end) / 2;
            const railHeight = Math.min(1, h - loft.elevation - 0.1);
            box(
              x,
              loft.elevation + railHeight,
              z,
              horizontal ? end - start : 0.045,
              0.045,
              horizontal ? 0.045 : end - start,
              "#706b60",
            );
            const posts = Math.max(1, Math.ceil((end - start) / 0.45));
            for (let i = 0; i <= posts; i++) {
              const offset = start + ((end - start) * i) / posts;
              box(
                horizontal ? loft.x + offset : x,
                loft.elevation + railHeight / 2,
                horizontal ? z : loft.z + offset,
                0.035,
                railHeight,
                0.035,
                "#706b60",
    ...
    space.furniture.forEach((f) => {
      ...
      g.position.set(
        f.x,
        space.loft?.furnitureIds.includes(f.id) ? space.loft.elevation : 0,
        f.z,
      );
    ...
    let yaw = 0.7,
      pitch = 0.55;
    const upstairs = active === "loft" && !!space.loft;
    const current = levelSpace(space, upstairs);
    const zone = current.zones.find((z) => z.id === active) || current.zones[0];
    const point = zone ? viewpoint(current, zone) : { x: w / 2, z: d / 2 };
    const elevation = upstairs ? space.loft!.elevation : 0;
    const underLoft =
      !upstairs &&
      space.loft &&
      point.x >= space.loft.x &&
      point.x <= space.loft.x + space.loft.width &&
      point.z >= space.loft.z &&
      point.z <= space.loft.z + space.loft.depth;
    const eyeHeight =
      elevation +
      Math.min(
        1.55,
        (underLoft ? space.loft!.elevation - 0.15 : h) - elevation - 0.3,
      );
    const updateCamera = () => {
      if (mode === "overview") {
        const radius = Math.max(w, d, h) * 1.85;
        camera.position.set(
          w / 2 + Math.sin(yaw) * Math.cos(pitch) * radius,
          h / 2 + Math.sin(pitch) * radius,
          d / 2 + Math.cos(yaw) * Math.cos(pitch) * radius,
        );
        camera.lookAt(w / 2, h / 2, d / 2);
      } else {
        camera.position.set(point.x, eyeHeight, point.z);
        camera.lookAt(
